...
class BWCircularSlider: UIControl {
    ...
    override init(frame: CGRect) {
        ...
        let textFieldRect = CGRect(x: (frame.size.width  - fontSize.width) / 2.0, y: (frame.size.height - fontSize.height) / 2.0, width: fontSize.width, height: fontSize.height)
    ...
    override func draw(_ rect: CGRect){
        ...
        let centerPoint = CGPoint(x: CGFloat(self.frame.size.width / 2.0), y: CGFloat(self.frame.size.height / 2.0))

        ctx?.addArc(center: centerPoint, radius: radius, startAngle: 0, endAngle: CGFloat(Double.pi * 2), clockwise: true)
        ...
        imageCtx?.addArc(center: CGPoint(x: CGFloat(self.frame.size.width/2), y: CGFloat(self.frame.size.height/2)), radius: radius, startAngle: 0, endAngle: CGFloat(DegreesToRadians(value: Double(angle))), clockwise: true)
        UIColor.red.set()
...
//        let startColorComps: UnsafePointer<CGFloat> = startColor.cgColor.components!
        let startColorComps: UnsafePointer<CGFloat> = UnsafePointer(startColor.cgColor.components!)
        let endColorComps: UnsafePointer<CGFloat> = UnsafePointer(endColor.cgColor.components!)
